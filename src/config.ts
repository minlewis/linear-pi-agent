...
const ConfigSchema = z.object({
  ...
  KIMI_MODEL_ROUTER: z.preprocess(emptyStringAsUndefined, z.string().trim().min(1).optional()),
  KIMI_MODEL_EASY: z.preprocess(emptyStringAsUndefined, z.string().trim().min(1).optional()),
  KIMI_MODEL_HARD: z.preprocess(emptyStringAsUndefined, z.string().trim().min(1).optional()),
...
export function publicConfig() {
  return {
    ...
    kimiModelRouter: config.KIMI_MODEL_ROUTER,
    kimiModelEasy: config.KIMI_MODEL_EASY,
    kimiModelHard: config.KIMI_MODEL_HARD,
